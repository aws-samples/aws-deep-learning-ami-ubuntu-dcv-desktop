...
class VLMConfig:
    ...
    def __post_init__(self):
        if self.data_dir is None and self.hf_dataset_name is not None:
            ...
            self.data_dir = str(
                Path.cwd() / f"datasets/{dataset_name}/{dataset_config}/train={train_pct}%-val={val_pct}%-test={test_pct}%"
            )
        elif self.data_dir is None:
            self.data_dir = str(Path.cwd() / "datasets/visual_instruct")
...
def train_func(config_dict: Dict):
    ...
    adapter = get_adapter_for_model(config.model_id)
    ...
    processor = adapter.load_processor(config.model_id)
